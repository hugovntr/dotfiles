...
local storage_path = wezterm.home_dir .. "/.cache/wezterm/sessions/"
...
local function save_to_json_file(data, file_path)
	...
	os.execute("mkdir -p $(dirname " .. file_path .. ")")
...
local function recreate_workspace(window, workspace_data)
	local function extract_path_from_dir(working_directory)
		...
		elseif platform == "x86_64-unknown-linux-gnu" then
	...
	for _, tab_data in ipairs(workspace_data.tabs) do
		...
		for j, pane_data in ipairs(tab_data.panes) do
			...
			if not (platform == "x86_64-pc-windows-msvc") then
				if not (platform == "x86_64-pc-windows-msvc") and pane_data.tty:sub(-#"/bin/nvim") == "/bin/nvim" then
...
function session_manager.restore_state(window)
	...
	local file_path = wezterm.home_dir
		.. "/.cache/wezterm/wezterm-session-manager/wezterm_state_"
		.. workspace_name
		.. ".json"
	file_path = storage_path .. "state_" .. workspace_name .. ".json"
...
function session_manager.save_state(window)
	...
	local file_path = wezterm.home_dir
		...
		.. data.name
		.. ".json"
	file_path = storage_path .. "state_" .. data.name .. ".json"
